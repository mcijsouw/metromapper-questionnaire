...
<html lang="en">
	...
	<body>
		...
		<div class="explanation">
			...
			<p class="align-left"><img class="left" src="/img/explanation/count4.jpg">In maps with a source based visualization, "important" lines (lines that are used most often when starting at station <strong>A</strong>) are displayed thicker than "unimportant" lines (lines that are rarely used). In the example on the left, <strong>A</strong> is the given starting station. </p>
			...
			<p class="align-left">Note that you should always follow your intuition, for instance if we want to travel from <strong>D</strong> to <strong>B</strong>, it is faster to go directly to <strong>B</strong> instead of taking a long detour via <strong>A</strong> and <strong>C</strong>.</p>
			<h4>Feature 3: Dotted lines</h4>
			<p class="align-left dotted-lines">
				<img class="left" src="/img/explanation/dotted-lines.jpg">If we want to find the fastest route from starting station <strong>A</strong> to any arbitrary destination station, we must avoid traveling over the dotted lines.
				<img class="avoid-dotted-lines" src="/img/explanation/avoid-dotted-lines.png">
			</p>
			<h4>Feature 4: Arrow hints</h4>
			<p class="align-left">
				<img class="left" src="/img/explanation/arrow-hints.jpg">If we want to find the fastest route from starting station <strong>A</strong> to any arbitrary destination station, we must <strong>only travel over line segments on which the arrow is pointing in the direction we are traveling</strong>. Line segments that are not annotated with an arrow should be avoided altogether. Note that this only applies to maps in which arrow hints are used.
